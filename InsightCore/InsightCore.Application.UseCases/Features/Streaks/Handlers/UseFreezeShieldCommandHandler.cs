...
{
    public class UseFreezeShieldCommandHandler : IRequestHandler<UseFreezeShieldCommand, Response<StudentStreakDto>>
    {
        private readonly ApplicationDbContext _context;
        private readonly IMediator _mediator;

        public UseFreezeShieldCommandHandler(ApplicationDbContext context, IMediator mediator)
        {
            _context = context;
            _mediator = mediator;
        }

        public async Task<Response<StudentStreakDto>> Handle(UseFreezeShieldCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (streak.FreezeShieldsAvailable <= 0)
                {
                    response.IsSuccess = false;
                    response.Message = "No freeze shields available.";
                    return response;
                }

                var log = streak.UseFreezeShield(request.ShieldDate);
                if (log != null)
                {
